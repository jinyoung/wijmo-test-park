<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-text>
            ...
            <RnakId offline label="RnakId" v-model="value.rnakId" :editMode="editMode" @change="change"/>
...
import RnakBase from '../components/RnakBase.vue'
...
export default {
    ...
    methods: {
        async init() {
            ...
            let RnakClass = me.$Vue.extend(RnakBase);
            me.rnakId = new RnakClass();
            ...
            let Promises = lists.map(async function (value) {
                ...
                if (value.rnakId && value.rnakId.id){
                    value.rnakId = await me.rnakId.getRealEntity(value.rnakId.id);
                }
